...
export default function RequestList() {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Dev mode toggle bar */}
      <div className="shrink-0 flex items-center justify-between px-4 py-2 border-b border-gray-700">
        <h2 className="text-xl font-bold">请求列表</h2>
      ...
      {devMode ? (
        <>
          {/* Main: DevPanel takes full area */}
          <div className="flex-1 min-h-0">
            <DevPanel />
          </div>
          {/* Bottom: request list strip (~2 cards) */}
          <div className="shrink-0 max-h-[180px] overflow-auto border-t border-gray-700 bg-gray-900/80 px-4 py-2 space-y-2">
            {records.slice(-3).reverse().map(r => (
              <div key={r.id}
                className={`bg-gray-800 rounded p-2 border text-xs ${
                  r.is_suspicious ? 'border-red-500/50' : 'border-gray-700'
                }`}
              >
                <div className="flex items-center justify-between gap-2">
                  <span className="font-medium text-gray-200 shrink-0">{r.model}</span>
                  <span className={r.is_suspicious ? 'text-red-400' : 'text-green-400 shrink-0'}>
                    {r.is_suspicious ? '可疑' : '正常'}
                  </span>
                  <span className="text-gray-500 truncate min-w-0">
                    Input {r.claimed_input_tokens}→{r.real_input_tokens}
                    {' · '}Output {r.claimed_output_tokens}→{r.real_output_tokens}
                  </span>
                </div>
              </div>
            ))}
            {records.length === 0 && (
              <div className="text-gray-500 text-center text-xs py-1">暂无审计记录</div>
            )}
          </div>
        </>
      ) : (
        /* Normal mode: full request list */
