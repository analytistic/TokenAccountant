...
export default function BarChart7Day({ data }: BarChart7DayProps) {
  ...
  // Compute layout based on actual container width
  const layout = useMemo(() => {
    const w = width || 400; // fallback
    const h = Math.max(140, w * 0.38); // maintain aspect ratio
    const pad = { top: 4, right: 6, bottom: 20, left: w * 0.07 };
  ...
  if (!groups.length) {
    return (
      <div ref={containerRef} className="flex items-center justify-center h-full text-sm text-gray-500">
        暂无统计数据
      </div>
    );
  }
